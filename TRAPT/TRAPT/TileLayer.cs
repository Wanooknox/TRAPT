...
namespace TRAPT
{
    public class TileLayer : Microsoft.Xna.Framework.GameComponent
    {
        ...
        /// </summary>
        public void Initialize(Texture2D texture, string contentPath)
        {
                        ...
                        foreach(char c in line.ToArray() )
                        {
                            try
                            {
                                ...
                                if (type == 1) //if wall
                                {
                                    ...
                                    mapData[currentX, currentY - 1].Initialize(new Vector2(currentX, currentY - 1), 0);
                                }
                                else if (type == 0) //if floor
                                {
                                    mapData[currentX, currentY - 1] = new FloorTile(Game);
                                    mapData[currentX, currentY - 1].Initialize(new Vector2(currentX, currentY - 1), 1);
                                }
                            }
                            catch(Exception e)
                            {
