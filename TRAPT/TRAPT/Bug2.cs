...
namespace TRAPT
{
    public class Bug2
    {
        ...
        public void MoveTowardsDestination(PathNode currentNode, List<WallTile> obstacles)
        {
            destination = currentNode.getPosition();
            ...
            mLine = new Line(currentNode.getPosition(), position);
            ...
            {
                if (AABB.Intersects(obstacles[i].Destination))
                {
                    collision = true;
                    collisionRectangle = new Rectangle(obstacles[i].Destination.X - AABB.Width / 2, obstacles[i].Destination.Y - AABB.Height / 2, obstacles[i].Destination.Width + AABB.Height, obstacles[i].Destination.Height + AABB.Height);
                }
            }
        ...
        public PathNode Update(PathNode currentNode, List<WallTile> obstacles)
        {
            if (!circumnavigate)
            {
